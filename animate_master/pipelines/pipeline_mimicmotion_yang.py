...
class MimicMotionPipeline_v2(DiffusionPipeline):
    r"""
    ...
    def _get_add_time_ids(
        ...
        passed_add_embed_dim = self.denoising_unet.config.addition_time_embed_dim * len(add_time_ids)
        # expected_add_embed_dim = self.unet.add_embedding.linear_1.in_features
        expected_add_embed_dim = self.denoising_unet.add_embedding.linear_1.in_features
        # 以上自己写
    ...
    def __call__(
        ...
        num_frames = num_frames if num_frames is not None else self.denoising_unet.config.num_frames
        # 以上自己改
        ...
        clip_image_embeds = self.image_enc(
            clip_image_tensor.to(device, dtype=self.image_enc.dtype)
        ).image_embeds
