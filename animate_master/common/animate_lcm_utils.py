...
class SVDSolver():
    def __init__(self, N, sigma_min, sigma_max, rho, Pmean, Pstd):
        ...
        self.sigmas = (sigma_max ** (1 / rho) + self.indices / (N - 1) * (
        ...
        self.weights = (1 / (self.sigmas[:-1] - self.sigmas[
                                                1:])) ** 0.1  # This is not optimal and can influence the training dynamics a lot. Wish someone can make it better.
    ...
    def sample_params(self, indices):
        ...
        sampled_weights = self.weights[
            torch.where(indices > self.weights.shape[0] - 1, self.weights.shape[0] - 1, indices)]
    ...
    def sample_timesteps(self, bsz):
        ...
        sampled_indices, sampled_sigmas, sampled_timesteps, sampled_weights, sampled_c_in, sampled_c_out, sampled_c_skip = self.sample_params(
            sampled_indices)
