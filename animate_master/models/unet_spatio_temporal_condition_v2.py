...
class UNetSpatioTemporalConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        for i, down_block_type in enumerate(down_block_types):
            ...
            #     dim_head=40,
            # )
            # torch.nn.init.zeros_(ref_attn.to_out[0].weight)
            # torch.nn.init.zeros_(ref_attn.to_out[0].bias)
            # self.down_ref_attns.append(ref_attn)
        ...
        # warp attention
        self.pose_warp_attn = Attention(
            query_dim=320,
            cross_attention_dim=4,
            heads=8,
            dim_head=40,
        )
        torch.nn.init.zeros_(self.pose_warp_attn.to_out[0].weight)
        torch.nn.init.zeros_(self.pose_warp_attn.to_out[0].bias)
    ...
    def forward(
        ...
        if pose_latents is not None:
            b, c, h, w = pose_latents.shape
            ref_latents_ = rearrange(ref_latents, "b c h w -> b (h w) c")
            pose_latents_ = rearrange(pose_latents, "b c h w -> b (h w) c")
            pose_ref_latents = self.pose_warp_attn(
                pose_latents_,
                encoder_hidden_states=ref_latents_
            )
            pose_ref_latents = rearrange(pose_ref_latents, "b (h w) c -> b c h w", h=h, w=w)
            sample = sample + pose_latents + pose_ref_latents
        ...
        for i, downsample_block in enumerate(self.down_blocks):
            ...
            # ref_latents_ = F.interpolate(ref_latents, (h, w), mode="bilinear", align_corners=True)
            # ref_latents_ = rearrange(ref_latents_, "b c h w -> b (h w) c")
            # sample_ = rearrange(sample, "b c h w -> b (h w) c")
            # sample_ref_latents = self.down_ref_attns[i](
            #     sample_,
            #     encoder_hidden_states=ref_latents_
            # )
            # sample_ref_latents = rearrange(sample_ref_latents, "b (h w) c -> b c h w", h=h, w=w)
            # sample = sample + sample_ref_latents
