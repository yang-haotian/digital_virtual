...
class ReferenceAttention(nn.Module):
    def __init__(self, in_dim, norm_eps=1e-6, norm_elementwise_affine=True):
        super().__init__()
        self.norm = nn.LayerNorm(in_dim, eps=norm_eps, elementwise_affine=norm_elementwise_affine)
        self.norm_refer = nn.LayerNorm(in_dim, eps=norm_eps, elementwise_affine=norm_elementwise_affine)

        self.refer_attn = Attention(
            query_dim=in_dim,
            heads=8,
            dim_head=40
        )
        torch.nn.init.zeros_(self.refer_attn.to_out[0].weight)
        torch.nn.init.zeros_(self.refer_attn.to_out[0].bias)

    def forward(self, x, x_refer):
        x_ = self.norm(x)
        x_refer_ = self.norm_refer(x_refer)
        out = self.refer_attn(x_, torch.cat([x_, x_refer_], dim=1))
        return out
...
class UNetSpatioTemporalConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        self.down_ref_attns = nn.ModuleList([])
        ...
        # down
        self.pose_down_blocks = nn.ModuleList([])
        self.ref_down_blocks = nn.ModuleList([])
        self.attn_down_blocks = nn.ModuleList([])
        ref_input_channels = in_channels // 2
        ...
        for i, down_block_type in enumerate(down_block_types):
            ...
            # ref down block
            ref_down_block = nn.Sequential(
                nn.Conv2d(ref_input_channels, output_channel, kernel_size=(3, 3), padding=1,
                          stride=(2, 2) if i < len(down_block_types) - 1 else (1, 1)),
                nn.SiLU(),
                nn.Conv2d(output_channel, output_channel, kernel_size=(3, 3), padding=1, stride=(1, 1)),
                nn.SiLU(),
            )
            ref_input_channels = output_channel
            ref_down_block.apply(init_weights)
            self.ref_down_blocks.append(ref_down_block)

            # pose down block
            pose_down_block = nn.Sequential(
            ...
            pose_down_block.apply(init_weights)
            self.pose_down_blocks.append(pose_down_block)

            # cross attention block
            refer_attn = ReferenceAttention(output_channel)
            self.attn_down_blocks.append(refer_attn)
    ...
    def forward(
        ...
        ref_latents = sample.chunk(2, dim=2)[-1]
        sample = sample.flatten(0, 1)
        ref_latents = ref_latents.flatten(0, 1)
        ...
        for i, downsample_block in enumerate(self.down_blocks):
            ...
            # v3 feature
            if pose_latents is not None:
                pose_latents = self.pose_down_blocks[i](pose_latents)
                sample += pose_latents

            h, w = sample.shape[-2:]
            ref_latents = self.ref_down_blocks[i](ref_latents)
            ref_latents_ = rearrange(ref_latents, "b c h w -> b (h w) c")
            sample_ = rearrange(sample, "b c h w -> b (h w) c")
            sample_ = self.attn_down_blocks[i](sample_, ref_latents_)
            sample_ = rearrange(sample_, "b (h w) c -> b c h w", h=h, w=w)
