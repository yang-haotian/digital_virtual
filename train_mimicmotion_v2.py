# -*- coding: utf-8 -*-
"""
CUDA_VISIBLE_DEVICES=0 accelerate launch train_mimicmotion_v2.py \
    --pretrained_model_name_or_path=checkpoints/stable-video-diffusion-img2vid-xt-1-1 \
    --pretrain_unet outputs/MimicMotion-20241111145333/checkpoint-27000/unet.pth \
    --pretrain_cond_net outputs/MimicMotion-20241111145333/checkpoint-27000/cond_net_smplx.pth \
    ...
    --learning_rate=1e-5 --lr_warmup_steps=100 \
    --seed=1234 \
...
def main():
    ...
    # process_group_kwargs = InitProcessGroupKwargs(timeout=timedelta(seconds=3600))  # 1 hours
    ...
    # unet
    # 遍历模型的参数
    trainable_names = ["pose_down_blocks", "ref_down_blocks", "attn_down_blocks"]
    # trainable_names = ['temporal_transformer_block']
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet, cond_net):
                ...
                if args.conditioning_dropout_prob is not None:
                    ...
                    mask_values_cond = image_mask[:, None] * mask_values_cond
                    ...
                    if (
                        ...
                        with torch.no_grad():
                            ...
                            with torch.autocast(
                                ...
                                with torch.no_grad():
                                    if args.cond_type in ["smplx"]:
                                        val_infos = {
                                            "data/yht_tests/demo1": ["move1.JPG", "IMG_8812-smplx.mp4", 30],
                                            ...
                                            "data/yht_tests/demo9": ["onana.JPG", "IMG_5494-smplx.mp4", 60],
                                            # "data/yht_tests/demo10": ["onna1.png", "IMG_8876-smplx.mp4", 30],
                                            # "data/yht_tests/demo11": ["onanav2.png", "throw-smplx.mp4", 30],
                                        }
                                    else:
                                        val_infos = {
                                    ...
                                    tile_size = model_pred.shape[1]
                                    tile_overlap = 6
                                    for val_dir in val_infos:
                                        val_name = os.path.basename(val_dir)
                                        ref_image = Image.open(os.path.join(val_dir, val_infos[val_dir][0]))
