"""
CUDA_VISIBLE_DEVICES=1 accelerate launch train_musepose_stage2_v2.py \
--config configs/train/musepose_train_stage2_v2.yaml
"""
import os
...
def main(cfg):
    ...
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=cfg.data.train_bs, shuffle=True, num_workers=min(cfg.data.train_bs, 8), drop_last=True
    )
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                if "mask_values_cond" in batch:
                    batch_size = mask_values_cond.shape[0]
                    mask_values_cond = rearrange(mask_values_cond, "b f c h w -> (b f) c h w")
                    mask_values_cond = F.interpolate(mask_values_cond, size=model_pred.shape[-2:], mode='nearest')
                    mask_values_cond = rearrange(mask_values_cond, "(b f) c h w -> b c f h w", b=batch_size)
