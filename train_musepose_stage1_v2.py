"""
CUDA_VISIBLE_DEVICES=0 accelerate launch train_musepose_stage1_v2.py \
--config configs/train/musepose_train_stage1_v2.yaml
"""
import os
...
from animate_master.datasets.musepose_image_dataset import MusePoseImageDataset
from animate_master.models.mutual_self_attention_v2 import ReferenceAttentionControl
...
from animate_master.pipelines.pipeline_musepose_image_v2 import MusePoseImagePipeline
...
def log_validation(
    ...
    with torch.autocast(
        ...
        if cfg.data.cond_type in ["smplx"]:
            val_infos = {
                "data/yht_tests/demo1": ["move1.JPG", "IMG_8812-smplx.mp4", 40],
                "data/yht_tests/demo2": ["player1v2.jpg", "IMG_7388-smplx.mp4", 60],
                "data/yht_tests/demo3": ["player2v3.jpg", "IMG_7387-smplx.mp4", 60],
                "data/yht_tests/demo4": ["img.png", "verse3725-smplx.mp4", 100],
                "data/yht_tests/demo8": ["img.png", "0725-smplx.mp4", 70],
                "data/yht_tests/demo9": ["onana.JPG", "IMG_5494-smplx.mp4", 70],
                "data/yht_tests/demo10": ["onna1.png", "IMG_8876-smplx.mp4", 60]
            }
        else:
            val_infos = {
                "data/yht_tests/demo1": ["move1.JPG", "IMG_8812-pose.mp4", 40],
                "data/yht_tests/demo2": ["player1v2.jpg", "IMG_7388-pose.mp4", 60],
                "data/yht_tests/demo3": ["player2v3.jpg", "IMG_7387-pose.mp4", 60],
                ...
                "data/yht_tests/demo10": ["onna1.png", "IMG_8876-pose.mp4", 60]
            }
        for val_dir in val_infos:
            val_name = os.path.basename(val_dir)
            ref_image_pil = Image.open(os.path.join(val_dir, val_infos[val_dir][0]))
            short_size = 576
            ...
            cond_images = utils.read_frames(os.path.join(val_dir, val_infos[val_dir][1]))
            pose_image = cond_images[start_ind]
            original_width, original_height = pose_image.size
            pose_image_pil = pose_image.resize((ow, oh))
            ...
            imgs = [ref_image_pil, pose_image_pil, image]
            x_offset = 0
            for i, img in enumerate(imgs):
                ...
                x_offset += img.size[0]
            img_save_path = os.path.join(save_dir, f"{val_name}_{seed}_{global_step}.jpg")
            image_grid.save(img_save_path)
            print(f"save validation {val_name} at: {img_save_path}")
...
def main(cfg):
    ...
    train_dataset = MusePoseImageDataset(
        ...
        cond_type=cfg.data.cond_type,
        cond_with_mask=[]
    )
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=cfg.data.train_bs, shuffle=True, num_workers=min(cfg.data.train_bs, 12),
        drop_last=False
    )
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                pixel_values_cond = pixel_values_cond.unsqueeze(2)  # (bs, 3, 1, 512, 512)
                with torch.no_grad():
                    ...
                    prompt_mask = random_p < cfg.uncond_ratio
                    image_prompt_embeds[prompt_mask] = 0
                    ref_image_latents[prompt_mask] = 0
                    if "mask_values_cond" in batch:
                        mask_values_cond[prompt_mask] = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./configs/train/musepose_train_stage1.yaml")
